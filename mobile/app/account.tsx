...
const website = (process.env.EXPO_PUBLIC_FATEDROP_WEB_URL || 'https://fatedrop.co.uk').replace(/\/$/, '');
...
export default function AccountScreen() {
  ...
  const submit = async () => {
    setLocalError(null);
    if (!email.trim() || !password) return setLocalError('Enter your FateDrop ID email and password.');
    try {
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <Text style={styles.lede}>Your account is shared across the website, mobile app and connected Discord access. Premium only unlocks when the backend membership record says it is active.</Text>
      ...
      {!signedIn ? <View style={styles.card}>
        ...
        <Pressable disabled={syncing} onPress={() => void submit()} style={styles.primary}><Text style={styles.primaryText}>{syncing ? 'Connecting…' : 'Sign in to FateDrop ID'}</Text></Pressable>
        <Pressable onPress={() => void Linking.openURL(`${website}/account/register`)} style={styles.secondary}><Text style={styles.secondaryText}>Create a FateDrop ID on the website ↗</Text></Pressable>
      </View> : <>
        <View style={styles.identityCard}>
          <View style={styles.avatar}><Text>FD</Text></View>
          <View style={{ flex: 1 }}><Text style={styles.handle}>{snapshot?.user.displayName || snapshot?.user.handle || 'Collector'}</Text><Text style={styles.fateId}>{snapshot?.user.fateId}</Text><Text style={styles.email}>{snapshot?.user.email}</Text></View>
          <View style={styles.status}><Text>{snapshot?.entitlement.effectiveTier.toUpperCase()}</Text></View>
        </View>

        <View style={styles.card}>
          <View style={styles.row}><View><Text style={styles.label}>MEMBERSHIP STATUS</Text><Text style={styles.value}>{snapshot?.entitlement.status || 'unknown'}</Text></View><Text style={snapshot?.entitlement.active ? styles.live : styles.free}>{snapshot?.entitlement.active ? 'ACTIVE' : 'FREE'}</Text></View>
          <Text style={styles.note}>The app never upgrades this locally. Payment confirmation updates the backend membership record; the next sync changes your effective tier and capabilities.</Text>
          <Pressable disabled={syncing} onPress={() => void refresh()} style={styles.secondary}><Text style={styles.secondaryText}>{syncing ? 'Syncing…' : 'Sync membership now'}</Text></Pressable>
          <Pressable onPress={() => void Linking.openURL(`${website}/subscriptions`)} style={styles.secondary}><Text style={styles.secondaryText}>Manage membership on website ↗</Text></Pressable>
        </View>

        <View style={styles.card}>
          <Text style={styles.label}>SERVER-CONFIRMED CAPABILITIES</Text>
          <View style={styles.capabilities}>{snapshot?.entitlement.capabilities.map((capability) => <View key={capability} style={styles.cap}><Ionicons name="checkmark-circle" size={14} color={FateDropColors.mint} /><Text>{capability.replaceAll('_', ' ')}</Text></View>)}</View>
        </View>

        {snapshot?.pendingMigrations.length ? <View style={styles.warning}><Text style={styles.warningTitle}>Sync foundation awaiting database migration</Text><Text style={styles.warningText}>{snapshot.pendingMigrations.join(', ')}</Text></View> : null}
...
const styles = StyleSheet.create({
  ...
  identityCard: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16, borderRadius: 19, backgroundColor: FateDropColors.glass, borderWidth: 1, borderColor: `${FateDropColors.violet}55`, marginBottom: 12 },
  ...
  status: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 999, backgroundColor: `${FateDropColors.mint}14` },
