...
function deltaLabel(value: number | undefined, rrp: number | undefined) {
  ...
  return `${sign}£${Math.abs(difference).toFixed(2)} · ${percentSign}${Math.abs(percent).toFixed(0)}% vs RRP`;
}
...
export default function TruePriceScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      } catch { setError('True Price could not be loaded from FateDrop Cloud.'); }
  ...
  const header = <>
    ...
    <View style={styles.sorts}><FilterChip label="Delivered price" active={sort === 'delivered'} onPress={() => setSort('delivered')} /><FilterChip label="Item price" active={sort === 'item'} onPress={() => setSort('item')} /></View>
...
const styles = StyleSheet.create({
  ...
  offer: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 11, borderTopWidth: 1, borderTopColor: FateDropColors.border }, retailer: { color: FateDropColors.text, fontWeight: '800' }, prices: { color: FateDropColors.secondary, fontSize: 10, marginTop: 3 }, delta: { color: FateDropColors.amber, fontSize: 9, fontWeight: '800', marginTop: 3 }, total: { color: FateDropColors.cyan, fontSize: 11, fontWeight: '800', marginTop: 5 }, deliveredDelta: { color: FateDropColors.secondary, fontSize: 9, marginTop: 2 }, collection: { color: FateDropColors.mint, fontSize: 9, marginTop: 3 }, buy: { width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: FateDropColors.violet },
