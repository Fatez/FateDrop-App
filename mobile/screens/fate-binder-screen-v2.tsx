...
import { addExactCardToCollector, confirmFateCollectorSetCompletion, fetchFateCollectorCollection, fetchFateCollectorSetProgress, previewFateCollectorSetCompletion, removeFateCollectorSetCompletion, updateFateCollectorItemQuantity, type FateCollectorItem, type FateCollectorMissingCard, type FateCollectorSetBinder, type FateCollectorSetCompletionPreview } from '@/services/fate-collector';
...
export default function FateBinderScreenV2() {
  const params = useLocalSearchParams<{ setId?: string | string[]; setName?: string | string[]; editionCode?: string | string[]; editionLabel?: string | string[] }>();
  const setId = first(params.setId)?.trim() || '';
  const routeSetName = first(params.setName)?.trim() || '';
  const editionCode = first(params.editionCode)?.trim() || 'standard';
  const editionLabel = first(params.editionLabel)?.trim() || (editionCode === 'standard' ? '' : editionCode.replaceAll('-', ' '));
  const request = useCallback(async () => { if (!setId) throw new Error('This binder does not have a set identity. Open it from Fate Collections.'); const [progress, collection] = await Promise.all([fetchFateCollectorSetProgress(setId, editionCode), fetchFateCollectorCollection()]); return { binder: progress.progress, items: collection.items }; }, [setId, editionCode]);
  const { data, loading, error, load } = useCollectionsResource(request, `${setId}|${editionCode}`);
  const binder = data?.binder;
  const { query, setQuery, sort, setSort, view, setView } = useBinderSession(`${setId}|${editionCode}`, binder?.setName || routeSetName);
  ...
  const editionItems = useMemo(() => (data?.items || []).filter((item) => { const variant = item.card?.variantCode || ''; if (editionCode === 'first-edition') return variant.startsWith('first-edition-'); if (editionCode === 'shadowless') return variant.startsWith('shadowless-'); return !variant.startsWith('first-edition-') && !variant.startsWith('shadowless-'); }), [data?.items, editionCode]);
  const entries = useMemo(() => binderEntries(editionItems, binder?.missingCards || [], setId, view, query, sort), [editionItems, binder, setId, view, query, sort]);
  const owned = useMemo(() => binderEntries(editionItems, [], setId, 'owned'), [editionItems, setId]);
  const checklistReady = binder?.status === 'available' && Number(binder.totalCount) > 0;
  const complete = isBinderComplete(binder);
  const setName = `${binder?.setName || routeSetName || 'Set binder'}${editionLabel ? ` · ${binder?.editionLabel || editionLabel}` : ''}`;
  ...
  const openSetCompletion = async () => {
    ...
    try {
      const preview = await previewFateCollectorSetCompletion(setId, editionCode);
  ...
  const confirmSetCompletion = async () => {
    ...
    setCompletionBusy(true);
    try {
      const result = await confirmFateCollectorSetCompletion(setId, completionPreview.confirmationToken, editionCode);
      setMessage(result.duplicate ? 'This verified checklist was already complete.' : `${result.createdCardItemCount} exact cards added to your collection. Existing copies were not duplicated.`);
  ...
  const removeSetCompletion = async () => {
    ...
    setCompletionBusy(true);
    try {
      await removeFateCollectorSetCompletion(setId, editionCode);
  ...
  return <CollectionsScreen><FlatList
    ...
    ListHeaderComponent={<>
      ...
      <FateMetricStrip items={[
        ...
        { icon: 'wallet-outline', value: money(binder?.value?.missingValue ?? binder?.value?.knownMissingValue, currency), label: binder?.value?.missingValue != null ? 'MISSING-CARD VALUE' : 'KNOWN MISSING VALUE', color: FateDropColors.goldBright },
      ...
      <View style={styles.progressTruth}><Ionicons name="calculator-outline" size={15} color={FateDropColors.goldBright} /><Text style={styles.progressTruthText}>{binder?.value?.missingExpectedCount != null ? binder.value.missingUnpricedCount === 0 ? `All ${binder.value.missingExpectedCount} missing slots have verified current prices. This is their market valuation; retailer prices and postage may differ.` : `${binder.value.missingPricedCount ?? 0} of ${binder.value.missingExpectedCount} missing slots are priced · ${binder.value.missingUnpricedCount ?? 0} still unpriced. FateDrop shows the known remainder and does not estimate the rest.` : 'Missing-card value appears as verified price evidence becomes available.'}</Text></View>
      ...
      {topMissing.length ? <>
        ...
        <View style={styles.topMissingRail}>
          {topMissing.map((card) => <Pressable key={`top:${card.fateCardId || card.printingId || card.collectorNumber}`} accessibilityRole="button" disabled={!card.fateCardId} onPress={() => card.fateCardId && router.push({ pathname: '/fate-price', params: { cardId: card.fateCardId, name: card.name || undefined, collectorNumber: card.collectorNumber || undefined, setId: card.setId, setName: card.setName || undefined, tcg: card.tcgCode || undefined } })} style={({ pressed }) => [styles.topMissingCard, pressed && card.fateCardId && styles.pressed, !card.fateCardId && styles.disabled]}>
    ...
    renderItem={({ item: entry }) => <View style={{ width: `${100 / columns}%`, paddingHorizontal: 4, paddingBottom: 10 }}>{entry.state === 'needed' ? <MissingCard card={entry.card} adding={Boolean(entry.card.fateCardId) && addingId === entry.card.fateCardId} disabled={Boolean(addingId) || loading || !entry.card.fateCardId} onAdd={() => void addMissing(entry.card)} refreshKey={data} /> : <OwnedCard item={entry.item} setId={setId} setName={setName} refreshKey={data} onChanged={() => void load()} />}</View>}
    ListFooterComponent={<>{!loading && !error && !entries.length ? <StateLine success={complete && !query.trim() && view === 'needed'} text={query.trim() ? 'No cards match that search.' : complete && view === 'needed' ? 'Nothing missing. Every verified regular card in this print run is represented.' : view === 'owned' ? 'No ungraded cards are recorded in this print run yet.' : checklistReady ? 'No card details are available for this view yet.' : 'Open Owned to see the cards already in your collection.'} /> : null}<View style={styles.truth}><Ionicons name="shield-checkmark-outline" size={16} color={FateDropColors.goldBright} /><Text style={styles.truthText}>Completion and value both use exact raw identities for this print run. Reverse holos and other master-set variants remain separate.</Text></View></>}
...
function MissingCard({ adding, card, disabled, onAdd, refreshKey }: { adding: boolean; card: FateCollectorMissingCard; disabled: boolean; onAdd: () => void; refreshKey: unknown }) {
  const exactIdentity = Boolean(card.fateCardId);
  ...
  const unresolved = !exactIdentity || card.identityStatus === 'printing_only_finish_or_edition_unresolved';
  const openPrice = () => {
    if (!card.fateCardId) return;
    router.push({ pathname: '/fate-price', params: { cardId: card.fateCardId, name: card.name || undefined, collectorNumber: card.collectorNumber || undefined, setName: card.setName || undefined, setId: card.setId, printingId: card.printingId || undefined, tcg: card.tcgCode || undefined } });
  };
  const openBuy = () => {
    if (!card.fateCardId) return;
    router.push({ pathname: '/fate-price-buy', params: { cardId: card.fateCardId, name: card.name || undefined, collectorNumber: card.collectorNumber || undefined, setName: card.setName || undefined, setId: card.setId, printingId: card.printingId || undefined, tcg: card.tcgCode || undefined } });
  };
  return <View style={[styles.card, styles.neededCard]}>
    <View style={styles.neededBadge}><Ionicons name="search-outline" size={10} color={FateDropColors.echo} /><Text style={styles.neededBadgeText}>NEEDED</Text></View>
    ...
    <View style={styles.cardBody}>
      ...
      <Text style={styles.cardMeta} numberOfLines={2}>{unresolved ? 'Binder verified · exact finish/edition pending' : card.rarity || card.variantCode || 'Exact printing'}</Text>
      <Text style={styles.cardPrice}>{unresolved ? 'PRICE UNAVAILABLE' : money(price?.amount, price?.currencyCode)}</Text>
      {exactIdentity ? <>
        <Pressable accessibilityRole="button" accessibilityLabel={`View price and history for ${card.name || 'this card'}`} onPress={openPrice} style={styles.priceButton}><Ionicons name="analytics-outline" size={14} color={FateDropColors.cyan} /><Text style={styles.priceButtonText}>PRICE & HISTORY</Text></Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel={`Buy ${card.name || 'this missing card'} from a verified retailer`} onPress={openBuy} style={styles.priceButton}><Ionicons name="storefront-outline" size={12} color={FateDropColors.cyan} /><Text style={styles.priceButtonText}>BUY THIS CARD</Text></Pressable>
      </> : <View style={styles.priceButton}><Ionicons name="shield-checkmark-outline" size={12} color={FateDropColors.goldBright} /><Text style={styles.priceButtonText}>EXACT IDENTITY HELD</Text></View>}
    </View>
    <Pressable disabled={disabled} onPress={onAdd} style={[styles.addButton, disabled && styles.disabled]}>{adding ? <ActivityIndicator size="small" color={FateDropColors.background} /> : <><Ionicons name={unresolved ? "lock-closed-outline" : "add"} size={14} color={FateDropColors.background} /><Text style={styles.addText}>{unresolved ? 'FINISH PENDING' : 'ADD OWNED'}</Text></>}</Pressable>
  </View>;
}
function OwnedCard({ item, setId, setName, refreshKey, onChanged }: { item: FateCollectorItem; setId: string; setName: string; refreshKey: unknown; onChanged: () => void }) { const [busy, setBusy] = useState(false); const price = useCollectionCardPrice(item.fateCardId, refreshKey); const card = item.card as (NonNullable<FateCollectorItem['card']> & ArtFields) | null | undefined; const art = card?.thumbnailUrl || card?.imageUrl || null; const addCopy = async () => { if (busy || item.quantity >= 999) return; setBusy(true); try { await updateFateCollectorItemQuantity(item.id, item.quantity + 1, item.revision); onChanged(); } finally { setBusy(false); } }; return <View style={[styles.card, styles.ownedCard]}><Pressable onPress={() => router.push({ pathname: '/fate-price', params: { cardId: item.fateCardId, name: card?.name || undefined, collectorNumber: card?.collectorNumber || undefined, setId, setName, tcg: card?.tcgCode || undefined } })} style={({ pressed }) => [styles.ownedCardMain, pressed && styles.pressed]}><View style={styles.ownedBadge}><Ionicons name="checkmark" size={10} color={FateDropColors.manifested} /><Text style={styles.ownedBadgeText}>OWNED ×{item.quantity}</Text></View><CanonicalThumbnail kind="card" setId={card?.setId || setId} collectorNumber={card?.collectorNumber} sourceUrl={art} width={76} height={106} /><View style={styles.cardBody}><Text style={styles.cardName} numberOfLines={2}>{card?.name || 'Verified card'}</Text><Text style={styles.cardNumber}>#{card?.collectorNumber || '—'}</Text><Text style={styles.cardMeta}>{item.conditionCode?.replaceAll('_', ' ') || 'condition unknown'}</Text><Text style={styles.cardPrice}>{money(price?.amount, price?.currencyCode)}{price ? ' each' : ''}</Text></View></Pressable><Pressable accessibilityRole="button" accessibilityLabel={`Add another ${card?.name || 'copy'}`} disabled={busy || item.quantity >= 999} onPress={() => void addCopy()} style={[styles.addCopyButton, busy && styles.disabled]}>{busy ? <ActivityIndicator size="small" color={FateDropColors.goldBright} /> : <Ionicons name="add" size={16} color={FateDropColors.goldBright} />}<Text style={styles.addCopyText}>ADD ANOTHER</Text></Pressable></View>; }
...
function SetCompletionModal({ visible, busy, binder, preview, setName, onClose, onConfirm, onRemove }: { visible: boolean; busy: boolean; binder?: FateCollectorSetBinder; preview: FateCollectorSetCompletionPreview | null; setName: string; onClose: () => void; onConfirm: () => void; onRemove: () => void }) {
  ...
  return <Modal visible={visible} transparent animationType="fade" statusBarTranslucent onRequestClose={onClose}><View style={styles.modalStage}><Pressable accessibilityRole="button" accessibilityLabel="Close set completion" onPress={onClose} style={StyleSheet.absoluteFill} /><View accessibilityViewIsModal style={styles.modalPanel}>
    ...
    <View style={styles.modalTruth}><Ionicons name="shield-checkmark-outline" size={18} color={FateDropColors.goldBright} /><Text style={styles.modalTruthText}>One verified exact raw card will be added for every missing regular slot. Existing copies are kept as-is and never duplicated. You can add more copies afterward.</Text></View>
