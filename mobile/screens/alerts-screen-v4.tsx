...
function liveWindowTime(value: string | null | undefined) { if (!value) return null; const date = new Date(value); if (!Number.isFinite(date.getTime())) return null; return date.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }); }
function liveWindowDuration(seconds: number | null | undefined) { if (seconds == null || !Number.isFinite(seconds) || seconds < 0) return null; const whole = Math.floor(seconds); if (whole < 60) return `${whole}s`; const minutes = Math.floor(whole / 60); const remainder = whole % 60; if (minutes < 60) return remainder ? `${minutes}m ${remainder}s` : `${minutes}m`; const hours = Math.floor(minutes / 60); const minuteRemainder = minutes % 60; return minuteRemainder ? `${hours}h ${minuteRemainder}m` : `${hours}h`; }
function liveWindowLine(alert: CanonicalMobileAlert) { if (alert.fateStage !== 'VANISHED' || !alert.liveWindow) return null; const start = liveWindowTime(alert.liveWindow.manifestedAt); const last = liveWindowTime(alert.liveWindow.lastConfirmedLiveAt); const end = liveWindowTime(alert.liveWindow.vanishedAt); const duration = liveWindowDuration(alert.liveWindow.observedDurationSeconds); if (alert.liveWindow.historyComplete && start && last && end && duration) return `Manifested ${start} · Last live ${last}\nVanished ${end} · Live ${duration}`; return `Manifested ${start ?? 'start not recorded'} · Last live ${last ?? 'unavailable'}\nVanished ${end ?? 'unavailable'} · ${duration ? `Live ${duration}` : 'Duration unavailable'}`; }
...
function AlertRow({ alert }: { alert: CanonicalMobileAlert }) {
  ...
  const liveWindow = liveWindowLine(alert);
  ...
  return <View style={styles.alertRow}>
    <Pressable onPress={openInApp} style={styles.alertBody}>
      ...
      <View style={styles.flex}>
        ...
        {liveWindow ? <Text style={[styles.alertLiveWindow, !alert.liveWindow?.historyComplete && styles.alertLiveWindowIncomplete]}>{liveWindow}</Text> : null}
      </View>
