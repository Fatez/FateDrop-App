    ...
    <View style={styles.safe}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={() => void load()} tintColor={FateDropColors.gold} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          ...
          <View style={styles.heroShade} /><View style={styles.fadeOne} /><View style={styles.fadeTwo} />
...
function AlertRow({ alert }: { alert: CanonicalMobileAlert }) { const item = meta[alert.fateStage]; return <Pressable onPress={() => alert.productUrl ? void Linking.openURL(alert.productUrl) : undefined} style={styles.alertRow}><View style={[styles.alertDot, { backgroundColor: item.color }]} /><View style={styles.flex}><View style={styles.alertTop}><Text style={[styles.alertStage, { color: item.color }]}>{item.label.toUpperCase()}</Text><Text style={styles.alertTime}>{ago(alert.detectedAt)}</Text></View><Text style={styles.alertTitle}>{alert.product?.title || alert.title}</Text><Text style={styles.alertMeta}>{alert.retailer || 'Retailer'} · {money(alert.product?.pricePence)}{alert.priceIntelligence?.rrpDeltaPercent != null ? ` · ${alert.priceIntelligence.rrpDeltaPercent > 0 ? '+' : ''}${alert.priceIntelligence.rrpDeltaPercent.toFixed(1)}% vs RRP` : ''}</Text></View><Ionicons name="chevron-forward" size={16} color={FateDropColors.muted} /></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  hero: { height: 350, overflow: 'hidden', backgroundColor: FateDropColors.background }, heroShade: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(4,6,11,.18)' }, fadeOne: { position: 'absolute', left: 0, right: 0, bottom: 0, height: 100, backgroundColor: 'rgba(4,5,11,.52)' }, fadeTwo: { position: 'absolute', left: 0, right: 0, bottom: 0, height: 46, backgroundColor: 'rgba(4,5,11,.82)' },
  ...
  heroCopy: { position: 'absolute', left: 20, right: 20, bottom: 24 }, heroEyebrow: { fontSize: 10, fontWeight: '900', letterSpacing: 1.3 }, heroTitle: { color: FateDropColors.ivory, fontFamily: Fonts?.serif, fontSize: 28, fontWeight: '700', marginTop: 4 }, heroSub: { color: FateDropColors.secondary, fontSize: 12, lineHeight: 18, marginTop: 6, maxWidth: 340 },
