...
const signalPresentation=(state:NetworkSignal['state'])=>{
  if(state==='whisper')return{label:'Whisper',color:FateDropColors.cyan,icon:'eye-outline' as const};
  if(state==='echo')return{label:'Echo',color:FateDropColors.violetLight,icon:'flash' as const};
  if(state==='manifested')return{label:'Manifested',color:FateDropColors.mint,icon:'sparkles' as const};
  return{label:'Vanished',color:FateDropColors.coral,icon:'close-circle' as const};
};
...
export default function AlertsScreenV2(){
  ...
  return <SafeAreaView style={styles.safe}><FateDropBackground/><ScrollView contentContainerStyle={styles.content}>
    ...
    <AbstractHero eyebrow="Alert network" title="From first movement to sold out." subtitle="Whisper spots product movement. Echo says get ready. Manifested means stock is live. Vanished means it has gone. Every stage stays linked to the same product." icon="notifications"/>

    <Header eyebrow="NETWORK LIFECYCLE" title="Whisper · Echo · Manifested · Vanished"/>
    <View style={styles.lifecycleGuide}>
      <Guide label="Whisper" text="Catalogue or product movement. Something may be coming." color={FateDropColors.cyan}/>
      <Guide label="Echo" text="Queue, traffic or security changed. Get ready." color={FateDropColors.violetLight}/>
      <Guide label="Manifested" text="Confirmed purchasable stock. Go now." color={FateDropColors.mint}/>
      <Guide label="Vanished" text="Confirmed availability is gone. Check alternatives." color={FateDropColors.coral}/>
    </View>
    ...
    }) : <Compact title={networkLoading?'Loading network activity…':'No lifecycle activity available'} text={networkError||'New Whisper, Echo, Manifested and Vanished events will appear here from FateDrop Cloud.'}/>}</View>
