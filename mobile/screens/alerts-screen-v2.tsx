...
const signalPresentation=(state:NetworkSignal['state'])=>state==='manifested'?{label:'Manifested',color:FateDropColors.mint,icon:'sparkles' as const}:state==='vanished'?{label:'Vanished',color:FateDropColors.coral,icon:'close-circle' as const}:{label:'Echo',color:FateDropColors.violetLight,icon:'flash' as const};
...
export default function AlertsScreenV2(){
  ...
  return <SafeAreaView style={styles.safe}><FateDropBackground/><ScrollView contentContainerStyle={styles.content}>
    ...
    <AbstractHero eyebrow="Alert network" title="Every signal leads somewhere." subtitle="Echo, Manifested and Vanished share one product identity, so tapping an alert opens the product comparison instead of a dead notification." icon="notifications"/>

    <Header eyebrow="NETWORK LIFECYCLE" title="Echo · Manifested · Vanished"/>
    <View style={styles.list}>{networkSignals.length?networkSignals.map(signal=>{const presentation=signalPresentation(signal.state);return <Pressable key={signal.id} onPress={()=>openSignal(signal)} style={[styles.networkCard,{borderColor:`${presentation.color}45`}]}><View style={[styles.signalIcon,{backgroundColor:`${presentation.color}16`}]}><Ionicons name={presentation.icon} size={17} color={presentation.color}/></View><View style={styles.copy}><Text style={[styles.signalLabel,{color:presentation.color}]}>{presentation.label.toUpperCase()}</Text><Text style={styles.cardTitle}>{signal.title}</Text><Text style={styles.cardDetail}>{signal.retailerName||'FateDrop network'} · {signalPrice(signal)} · {ago(signal.detectedAt)}</Text>{signal.reason?<Text style={styles.signalReason}>{signal.reason}</Text>:null}</View><Ionicons name="chevron-forward" size={17} color={FateDropColors.muted}/></Pressable>}:<Compact title={networkLoading?'Loading network activity…':'No lifecycle activity available'} text={networkError||'New Echo, Manifested and Vanished events will appear here from FateDrop Cloud.'}/>}</View>
