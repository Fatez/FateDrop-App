...
const REQUIRED_CLIPS = ['Idle', 'Echo', 'Notice', 'Manifested', 'Celebrate', 'Walk', 'Run'] as const;
...
/**
 * Three's WebGLRenderer normally expects a DOM canvas. Expo GL provides the
 * WebGL context directly, so give Three only the minimal canvas surface it
 * queries. This is the small, dependency-free bridge previously supplied by
 * expo-three's Renderer class.
 */
...
async function loadCompanionGltf(url: string): Promise<GLTF> {
  ...
  if (!response.ok) throw new Error(`Companion asset HTTP ${response.status}.`);

  const buffer = await response.arrayBuffer();
  if (buffer.byteLength < 20) throw new Error('Companion GLB response was empty or invalid.');
...
async function createScene(
  ...
  const loaded = await loadCompanionGltf(modelUrl);
  if (disposed()) {
    renderer.dispose();
    return () => undefined;
  }
  ...
  const missing = REQUIRED_CLIPS.filter((name) => !clipNames.has(name));
  if (missing.length) {
