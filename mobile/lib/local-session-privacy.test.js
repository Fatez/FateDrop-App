...
test('bearer token remains in SecureStore while legacy plaintext tokens are migrated away', () => {
  ...
  assert.match(service, /AsyncStorage\.removeItem\(TOKEN_KEY\)/);
});

test('failed remote logout revocation protection remains intact', () => {
  assert.match(service, /PENDING_REVOKE_KEY/);
  assert.match(service, /SecureStore\.setItemAsync\(PENDING_REVOKE_KEY,token\)/);
  assert.match(service, /retryPendingSessionRevocation\(\)/);
});
