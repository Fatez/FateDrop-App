...
test('primary navigation is Home, Alerts, FateDrop compass, Fate Market and Profile', () => {
  ...
  assert.match(marketRoute, /area === 'pulse'[\s\S]*href="\/fate-pulse"/);
  assert.match(marketRoute, /area === 'price'[\s\S]*href="\/fate-price"/);
  assert.match(marketRoute, /area === 'collectors'[\s\S]*href="\/collection"/);
