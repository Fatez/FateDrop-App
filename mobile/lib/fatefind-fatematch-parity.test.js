...
test('FateFind preserves the proven True Price comparison search and value verdict', () => {
  assert.match(fateFind, /\/api\/true-price\?q=/);
  assert.match(fateFind, /Find the best value before you buy/);
  assert.match(fateFind, /compareValueGroups/);
  assert.match(fateFind, /FATE VALUE COMPARE/);
  assert.match(fateFind, /RRP\/reference position decides value first/);
  assert.doesNotMatch(fateFind, /fetchLiveFateFind/);
