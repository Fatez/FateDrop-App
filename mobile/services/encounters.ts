...
import type { CalendarEvent, LocalRadarResponse } from '@/types/encounter';
...
export async function loadEncounter(id:string):Promise<CalendarEvent>{
  ...
  if(!data.event)throw new Error('Encounter not found');
  return data.event;
}

export async function loadLocalRadar(query:RadarQuery):Promise<LocalRadarResponse>{
  const params=new URLSearchParams();
  if(Number.isFinite(query.latitude))params.set('lat',String(query.latitude));
  if(Number.isFinite(query.longitude))params.set('lng',String(query.longitude));
  if(query.postcode?.trim())params.set('postcode',query.postcode.trim().toUpperCase());
  if(Number.isFinite(query.radiusMiles))params.set('radiusMiles',String(query.radiusMiles));
  if(query.tcg)params.set('tcg',query.tcg);
  params.set('types',(query.types?.length?query.types:['shops','events']).join(','));
