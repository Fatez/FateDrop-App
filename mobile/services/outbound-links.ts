...
export async function openTrackedRetailerLink(input:{destinationUrl:string;retailerId:string;offerId?:string;placement:string}){
  ...
  const supported=await Linking.canOpenURL(destinationUrl);if(!supported)throw new Error('This retailer link cannot be opened.');await Linking.openURL(destinationUrl);
}