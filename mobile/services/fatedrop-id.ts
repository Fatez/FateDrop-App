...
export type CrossPlatformFateMatchWatch = Record<string, unknown> & { id:string; userId:string; enabled:boolean };
/** @deprecated Legacy sync name for a FateMatch watch. */
export type CrossPlatformFateFind = CrossPlatformFateMatchWatch;
...
export type FateDropSyncSnapshot = { contractVersion:1; syncedAt:number; user:FateDropIdentity; entitlement:FateDropEntitlement; wishlist:CrossPlatformWishlistItem[]; fateMatchWatches:CrossPlatformFateMatchWatch[]; fateFinds:CrossPlatformFateFind[]; fateMatches:CrossPlatformFateMatch[]; notificationPreferences:CrossPlatformNotificationPreferences; pendingMigrations:string[] };
...
function normalizeSnapshot(snapshot:FateDropSyncSnapshot):FateDropSyncSnapshot{
  const fateMatchWatches=snapshot.fateMatchWatches||snapshot.fateFinds||[];
  return {...snapshot,fateMatchWatches,fateFinds:fateMatchWatches,fateMatches:snapshot.fateMatches||[],notificationPreferences:normalizePreferences(snapshot.notificationPreferences)};
}
...
export async function signInFateDropId(email:string,password:string){const response=await fetch(`${baseUrl()}/api/mobile/session`,{method:'POST',headers:{'content-type':'application/json',accept:'application/json'},body:JSON.stringify({email,password})});const result=await parseJson<LoginResponse>(response);await storeSessionToken(result.sessionToken);await AsyncStorage.removeItem(TOKEN_KEY);const fateMatchWatches=result.fateMatchWatches||result.fateFinds||[];return saveSnapshot({contractVersion:1,syncedAt:Math.floor(Date.now()/1000),user:result.user,entitlement:result.entitlement,wishlist:result.wishlist||[],fateMatchWatches,fateFinds:fateMatchWatches,fateMatches:result.fateMatches||[],notificationPreferences:normalizePreferences(result.notificationPreferences),pendingMigrations:result.pendingMigrations||[]});}
